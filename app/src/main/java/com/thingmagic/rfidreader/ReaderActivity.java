...
public class ReaderActivity extends TabActivity
{
    ...
    private static TabHost.TabSpec read_TabSpec = null;
    private static TabHost.TabSpec write_TabSpec = null;
    private static TabHost.TabSpec search_TabSpec = null;
...
//	EpcDatabase dbEPC = null;
	...
	public void onCreate(Bundle savedInstanceState) 
	{
		...
		writeButton.setOnClickListener(new WriteListener(this));
		readCurrentEPC.setOnClickListener(new CurrentEpcListener(this));
		...
		searchButton.setOnClickListener(new ServiceListener(this).SearchListener);
	...
	private void findAllViewsById()
	{
        ...
        customReaderRadioButton = (RadioButton) findViewById(R.id.CustomReader_radio_button);
        ...
        writeSelection1 = (RadioButton) findViewById(R.id.write_selection1);
        writeSelection2 = (RadioButton) findViewById(R.id.write_selection2);
        currentEPC = (EditText) findViewById(R.id.current_epc);
        productCode = (EditText) findViewById(R.id.productcode);
        readCurrentEPC = (Button) findViewById(R.id.Read_current_epc);
		...
		read_TabSpec = tabHost.newTabSpec("Read Tag");
		read_TabSpec.setContent(R.id.tab1);
		read_TabSpec.setIndicator("Read Tag");

		write_TabSpec = tabHost.newTabSpec("Write Tag");
		write_TabSpec.setContent(R.id.tab2);
		write_TabSpec.setIndicator("Write Tag");

		search_TabSpec = tabHost.newTabSpec("Search Tag");
		search_TabSpec.setIndicator("Seach Tag");
		search_TabSpec.setContent(R.id.tab3);

		tabHost.addTab(read_TabSpec);
		tabHost.addTab(write_TabSpec);
		tabHost.addTab(search_TabSpec);
		...
		public void onClick(View v) 
		{
			...
			if (radioButton.getText().equals("SerialReader")) 
			{
				try
				{
					...
					customReaderField.setVisibility(View.GONE);
					readEditText.setVisibility(View.GONE);
					serialList.setVisibility(View.VISIBLE);
				}
				catch(Exception ex)
				{
			...
			else if(radioButton.getText().equals("CustomReader"))
            {
                serialList.setVisibility(View.GONE);
                readEditText.setVisibility(View.GONE);
                customReaderField.setVisibility(View.VISIBLE);
            }
		}
	};
